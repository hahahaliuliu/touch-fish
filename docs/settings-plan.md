...
- 程序会自动扫描并验证所有词书，优先按 `activeVocabularyBook` 的词库 id 加载；找不到时回退到第一本可用词书
- 若没有任何词书，启动 `touchfish word` 会打开 Settings，以便下载或导入词书
...
### Vocabulary Download / 词库下载

当前已经支持：

- 在 Settings 中打开 `Download Vocabulary` 子界面
- 从公开 HTTPS 目录读取可下载词书
- 下载后校验词库格式和 id，再原子安装到本地词库目录
- 已安装的词书不会被静默覆盖
- 下载列表会显示已安装、可下载和计划中的词书；手动导入的本地 JSON、TXT、CSV、PDF 词书也会显示在已安装列表
- 已安装词书按 `Enter` 后必须按 `Y` 确认才会卸载；卸载会删除词书文件及其独立进度
- TXT、CSV 与可提取文本的 PDF 会在导入时转换为标准 JSON 词书；扫描件和复杂版式 PDF 需要后续的可预览、可校对流程
- 词书管理子界面会跟随 `interfaceLanguage` 显示中英文操作提示；词书名称、来源描述和许可证信息保留原文

当前公开目录默认只有一个 MIT 许可的示例词书。完整词书必须先确认来源允许公开再分发，才可以加入目录。
...
- 在 Word Session 按 `T`，使用当前学习组开始测试
- 默认显示英文、输入中文；`Tab` 切换为显示中文、输入英文
- 题目在当前学习组内随机排序，不影响背词顺序或学习进度
- 输入答案后按 `Enter` 判定；完成后显示正确数、错误数、正确率和全部错误单词
- `Enter` 或 `Esc` 从结果页返回原来的 Word Session

当前先不保存错题或统计，避免在测试功能稳定前提前引入 Review 数据结构。
